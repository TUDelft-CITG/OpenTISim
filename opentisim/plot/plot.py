...
def cashflow_plot(cash_flows, title='Cash flow plot', width=0.2, alpha=0.6, fontsize=20):
    ...
    material_costs = cash_flows['capex_material'].values + cash_flows['purchase_material'].values 
    H2_costs = cash_flows['purchaseH2'].values
...
#             profits_cum[index] = profits_cum[index - 1] + profits[index]
    ...
    ax1.bar([x - width for x in years], -opex, zorder=1, width=width, alpha=alpha, label="opex", color=colors[0],
            edgecolor='darkgrey')
    ax1.bar([x + width for x in years], -material_costs, zorder=1, width=width, alpha=alpha, label="material costs", color=colors[2],
        edgecolor='darkgrey')
    ax1.bar([x + 2*width for x in years], -H2_costs, zorder=1, width=width, alpha=alpha, label="H2 costs", color=colors[3],
        edgecolor='darkgrey')
