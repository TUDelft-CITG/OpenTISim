...
def quay_invest_decision(quays, berths, vessels, year, timestep):
    ...
    if invest_decision == 'Invest in quay':
        ...
        wave_motion   = 0.5
        safety_margin = 0.5
        quays[index].depth = vessel_draft + max_sinkage + wave_motion + safety_margin
...
def berth_invest_decision(berths, cranes, commodities, vessels, allowable_waiting_factor, year, timestep, operational_hours):
    ...
    # Determine total berth capacity
    capacity = []
    for i in range (len(berths)):
        if berths[i].online_date <= year:
...
def storage_invest_decision(storage, trigger_throughput_perc, aspired_throughput_perc, storage_type, commodities, year, timestep):
    ...
    if current_capacity < current_demand * trigger_throughput_perc and year < commodities[0].years[-2] and timestep > 0:
    ...
    for i in range(2):
        for j in range(len(storage[i])):
            ...
            # Utilization
            if matrix[-1,3] != 0:
                matrix[-1,4] = round(matrix[-1,2]/matrix[-1,3],2)
            else:
                matrix[-1,4] = 0
            # Translate to dataframe
            df = pd.DataFrame(matrix, columns=['Year', 'Commodity demand', 'Storage', 'Capacity', 'Forecasted utilization'])
...
def quay_conveyor_invest_decision(q_conveyors, berths, commodities, year, timestep, operational_hours):
