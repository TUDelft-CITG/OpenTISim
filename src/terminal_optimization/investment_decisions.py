...
def quay_invest_decision(quays, berths, vessels, year, timestep):
    ...
    if invest_decision == 'Invest in quay':
        added_length = []
        for i in range(-1 * berths[0].delta, 0):
            added_length.append(berths[i].length)
        # Add quay object
        quays.append(infra.quay_wall_class(**infra.quay_data))
        quays[-1].purchase_date = year
        quays[-1].online_date = year + quays[0].delivery_time
        quays[-1].length = np.sum(added_length)
        
        # Determine quay depth
        vessel_draft  = vessels[2].draft
        max_sinkage   = 0.5
        wave_motion   = 0.5
        safety_margin = 0.5
        quays[-1].depth = vessel_draft + max_sinkage + wave_motion + safety_margin
        
        # Refresh pending lengths
        for i in range (len(quays)):
            quays[i].offline_length = np.sum(added_length)
            quays[i].delta = np.sum(added_length)
    ...
    for i in range(len(quays)):
        ...
        if online_length:
            ...
            matrix[-1,1] = 0
        # Depth
        matrix[-1,2] = int(round(quays[i].depth))
        # Freeboard 
        matrix[-1,3] = int(round(quays[i].freeboard))
...
def berth_invest_decision(berths, cranes, commodities, vessels, allowable_waiting_factor, year, timestep, operational_hours):
    ...
    demand = commodities[0].demand[len(commodities[0].historic) + timestep] * commodities[0].utilization
    ...
    for i in range (3):
        ...
        if timestep == 0: 
            vessels[i].call_log = []
        vessels[i].n_calls = np.ceil(demand * percentage / vessels[i].call_size)
    ...
    if len(berths) == 0 or (max_waiting_factor > allowable_waiting_factor and year < commodities[0].years[-2]):
            ...
            for i in range(len(available_slots)):
                if int(available_slots[i]) != 0:
                    if berths[i].crane_type == 'Gantry cranes':
                        ...
                        if int(available_slots[i]) != berths[i].max_cranes:
                            cranes[0][-1].online_date = year + cranes[0][-1].delivery_time
                        else:
                            cranes[0][-1].online_date = year + berths[0].delivery_time
                        ...
                        if int(available_slots[i]) != berths[i].max_cranes:
                            cranes[1][-1].online_date = year + cranes[1][-1].delivery_time
                        else:
                            cranes[1][-1].online_date = year + berths[0].delivery_time
                        ...
                        if int(available_slots[i]) != berths[i].max_cranes:
                            cranes[2][-1].online_date = year + cranes[2][-1].delivery_time
                        else:
                            cranes[2][-1].online_date = year + berths[0].delivery_time
                        berths[i].cranes_present.append('Mobile crane')
                    elif berths[i].crane_type == 'Screw unloaders':
                        screw_unloaders_added.append(1)
                        cranes[3].append(infra.continuous_unloader(**infra.continuous_screw_data))
                        cranes[3][len(cranes[3])-1].berth = i+1
                        cranes[3][len(cranes[3])-1].purchase_date = year
                        if int(available_slots[i]) != berths[i].max_cranes:
                            cranes[3][-1].online_date = year + cranes[3][-1].delivery_time
                        else:
                            cranes[3][-1].online_date = year + berths[0].delivery_time
    ...
    for i in range (len(berths)):
        if berths[i].online_date <= year:
            ...
            online_berth_service_rate.append(0)
    ...
    for i in range (3):
        if traffic_ratio:
            vessels[i].call_log.append(
                vessels[i].n_calls * traffic_ratio[0])
        else:
            vessels[i].call_log.append(0)
    ...
    for i in range (len(berths)):
        if berths[i].online_date <= year:
            ...
            capacity.append(int(eff_unloading_cap * operational_hours * berths[0].waitingfactor_to_occupancy(allowable_waiting_factor, online_berths)))
        if berths[i].online_date > year:
            capacity.append(0)
    ...
    for i in range (len(berths)):
        if berths[i].online_date <= year:
            berth_occupancy   = occupancy[i]
            eff_unloading_cap = round(online_berth_service_rate[i])
            waiting_factor    = berths[0].occupancy_to_waitingfactor(berth_occupancy, online_berths)
            slots_available   = available_slots[i]
    ...
    if invest_decision == 'Do not invest in berths or cranes':
        berths[0].delta = 0
        for i in range (4):
            for j in range (len(cranes[i])):
                cranes[i][j].delta = 0
...
def storage_invest_decision(storage, dwell_time, vessels, storage_type, commodities, year, timestep, operational_hours):
    
    maize   = commodities[0]
    soybean = commodities[1]
    wheat   = commodities[2]
    
    current_demand = commodities[0].demand[len(commodities[0].historic) + timestep] * commodities[0].utilization
    
    # Convert dwell times to required capacity
    throughput_perc = dwell_time * 24 / operational_hours
    ...
    # Storage has to be able to store at least a single call_size of the largest vessel expected to call to port
    call_size = vessels[1].call_size
    throughput = max(call_size, current_demand * throughput_perc)
    
    # Determine whether to invest
    if current_capacity < throughput and year < commodities[0].years[-2] and timestep > 0:
    ...
    if invest_decision == 'Invest in storage':
        shortcoming = int(round((max(call_size, current_demand * throughput_perc * 1.10) - current_capacity)))
    ...
    for i in range(2):
        for j in range(len(storage[i])):
            # Register storage characteristics under the storage instance
            ...
            # Capacity
            if storage[i][j].online_date <= year:
                matrix[-1,3] = int(round(total_online_capacity/throughput_perc))
            else:
                matrix[-1,3] = 0
            # Utilization
            if matrix[-1,3] != 0:
...
def station_invest_decision(stations, trains, allowable_waiting_factor, commodities, timestep, year, operational_hours):
    ...
    for i in range (len(stations)):
        if i < len(pending_berth_service_rate):
            ...
            capacity       = int(round(operational_hours * stations[i].production * trains.waitingfactor_to_occupancy(allowable_waiting_factor, len(stations))))
        ...
        if online_stations == 0:
            calls_per_station = 0
        if online_stations != 0:
            calls_per_station = trains.calls / online_stations
        ...
        # Number of stations
        matrix[-1,6] = int(round(online_stations))
        # Calls per stations
        matrix[-1,7] = calls_per_station
        # Translate to dataframe
        df = pd.DataFrame(matrix, columns=['Year', 'Commodity demand', 'Total yearly loading capacity', 'Occupancy', 'Loading capacity', 'Waiting factor', 'Nr of stations', 'Calls per station'])
        # Register under berth instance
    ...
    if timestep > 0 and len(stations) == 0:
        invest_decision = 'Invest in stations'
    elif timestep > 0 and max_waiting_factor > allowable_waiting_factor and year < commodities[0].years[-2]:
        invest_decision = 'Invest in stations'
    else:
    ...
    # Determine total station capacity
    capacity = []
    for i in range (online_stations):
        if stations[i].online_date <= year:
            loading_rate       = stations[i].production
            loading_time       = trains.call_size/loading_rate
            prep_time          = trains.prep_time
            train_cycle        = loading_time + prep_time
            loading_perc       = loading_time / train_cycle
            max_occupancy      = trains.waitingfactor_to_occupancy(allowable_waiting_factor, online_stations)
            total_loading_time = operational_hours * max_occupancy * loading_perc
            yearly_capacity    = loading_rate * total_loading_time
            capacity.append(int(yearly_capacity))
        if stations[i].online_date > year:
            capacity.append(0)
    capacity = np.sum(capacity)  
...
def quay_conveyor_invest_decision(q_conveyors, berths, cranes, commodities, year, timestep, operational_hours):
    ...
    # Determine the peak capacity of each berth (including upcoming berths)
    pending_berth_service_rate = []
    for i in range (len(berths)):
        service_rate = []
        if berths[i].online_date <= year+1:
            for j in range(4):
                for k in range(len(cranes[j])):
                    if cranes[j][k].berth == i+1 and cranes[j][k].online_date <= year+1:
                        service_rate.append(cranes[j][k].peak_capacity)                  
            pending_berth_service_rate.append(int(np.sum(service_rate)))
    total_service_rate = np.sum(pending_berth_service_rate)
    ...
    quay_demand = int(round(total_service_rate))
    
    if capacity < quay_demand and year < commodities[0].years[-2]:
        ...
        if berths[i].online_date <= year:
            ...
            for j in range(4):
                for k in range(len(cranes[j])):
                    if cranes[j][k].berth == i+1 and cranes[j][k].online_date <= year:
                        service_rate.append(cranes[j][k].peak_capacity)                  
            online_service_rate.append(int(np.sum(service_rate)))
    total_service_rate = int(round(np.sum(online_service_rate)))
        
    for i in range(online_assets):    
        # Register berth characteristics under the berth instance
        matrix = np.zeros(shape=(1, 3))
        # Year
        matrix[-1,0] = int(round(year))
        # Quay demand
        matrix[-1,1] = total_service_rate
        # Capacity
        matrix[-1,2] = int(round(online))
        # Translate to dataframe
        df = pd.DataFrame(matrix, columns=['Year', 'Quay capacity demand', 'Capacity'])
...
def hinterland_conveyor_invest_decision(h_conveyors, stations, commodities, year, timestep, operational_hours):
    ...
    if capacity < demand and year < commodities[0].years[-2]:
