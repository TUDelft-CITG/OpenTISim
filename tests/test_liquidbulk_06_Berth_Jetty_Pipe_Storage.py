...
def test_liquidbulk_06_Berth_Jetty_Pipe_Storage():
	"""Test to see if for a given demand scenario and vessel mix the expected number of
	terminal elements are suggested if a Berth, Jetty, Pipeline and storage are build, check also the berth occupancy, waiting  	factor, throughput and costs - in this testcase the lifecycle is run in year-by-year stepping. Checks have been done for    	years 2023 and 2027.
	"""
	import pandas as pd
	import opentisim
	...
	# storage data 
	opentisim.liquidbulk.storage_lh2_data['delivery_time'] = 1 
	opentisim.liquidbulk.storage_lh2_data['lifespan'] = 30  #
	opentisim.liquidbulk.storage_lh2_data['unit_rate'] = 200_000_000  
	opentisim.liquidbulk.storage_lh2_data['mobilisation_min'] = 200_000  
	opentisim.liquidbulk.storage_lh2_data['mobilisation_perc'] = 0.003      
	opentisim.liquidbulk.storage_lh2_data['maintenance_perc'] = 0.01 
	opentisim.liquidbulk.storage_lh2_data['crew_min'] = 3  
	opentisim.liquidbulk.storage_lh2_data['crew_for5'] = 1    
	opentisim.liquidbulk.storage_lh2_data['insurance_perc'] = 0.01  
	opentisim.liquidbulk.storage_lh2_data['consumption'] = 610        
	opentisim.liquidbulk.storage_lh2_data['capacity'] = 3_540
	...
	Terminal = opentisim.liquidbulk.System(
		...
		terminal_supply_chain={'berth_jetty','pipeline_jetty_-_terminal','storage'}, #Choose what elements are on the terminal, 		other elements could be: 'mch_2_h2_retrieval','pipeline_terminal_-_hinterland'},
	...
	assert len(Terminal.elements) == 62
	assert len(opentisim.core.find_elements(Terminal, opentisim.liquidbulk.Storage)) == 48
	...
	for index, year in enumerate(years): 
		...
		for element in opentisim.core.find_elements(Terminal, opentisim.liquidbulk.Storage):
		...
		if index == 0: 
			...
			assert storage_cap_online == 2013662
			...
			assert storage_cap_online == 4027325
